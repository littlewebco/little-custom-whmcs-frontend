{if !$inShoppingCart}
    <div class="content-container-full clearfix">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    {$footeroutput}
                </div>
            </div>
        </div>
    </div>
{/if}

<section id="footer">
    <!-- Footer -->
    <div class="global-footer">
        <div class="container">
            <div class="footer-grid">
                <!-- Brand Section -->
                <div class="footer-brand">
                    <a href="{$WEB_ROOT}">
                        {if $assetLogoPath}
                            <img src="{$assetLogoPath}" alt="{$companyname}" width="47" height="47">
                        {else}
                            <img src="{$WEB_ROOT}/assets/img/logo.svg" alt="{$companyname}" width="47" height="47">
                        {/if}
                        <span>{$companyname}</span>
                    </a>
                    <p>Our mission is to make life easier for our customers. We do it by offering easy to use, fast and reliable technology and support services.</p>
                </div>

                <!-- Navigation Links -->
                <div class="footer-nav">
                    <div class="nav-column">
                        <a href="{$WEB_ROOT}/about">About Us</a>
                        <a href="{$WEB_ROOT}/services">Services</a>
                        <a href="{$WEB_ROOT}/ai-protection">AI Protection</a>
                        <a href="{$WEB_ROOT}/managed-services">Managed Services</a>
                        <a href="{$WEB_ROOT}/cyber-security">Cyber Security</a>
                    </div>
                    <div class="nav-column">
                        <a href="{$WEB_ROOT}/whois">WHOIS</a>
                        <a href="{$WEB_ROOT}/domain-tracker">Domain Tracker</a>
                        <a href="{$WEB_ROOT}/terms">Terms of Service</a>
                        <a href="{$WEB_ROOT}/privacy">Privacy Policy</a>
                        <a href="{$WEB_ROOT}/faq">FAQ</a>
                    </div>
                </div>

                <!-- Contact Section -->
                <div class="footer-contact">
                    <h3>Contact Us</h3>
                    <a href="mailto:helpdesk@little.cloud" class="contact-email">helpdesk@little.cloud</a>
                </div>
            </div>
        </div>
    </div>
</section>

{$footeroutput}

<section id="footer">
    <!-- Footer -->
    <div class="global-footer">
        <div class="container">
            <div class="footer-grid">
                <!-- Brand Section -->
                <div class="footer-brand">
                    <a href="{$WEB_ROOT}">
                        {if $assetLogoPath}
                            <img src="{$assetLogoPath}" alt="{$companyname}" width="47" height="47">
                        {else}
                            <img src="{$WEB_ROOT}/assets/img/logo.svg" alt="{$companyname}" width="47" height="47">
                        {/if}
                        <span>{$companyname}</span>
                    </a>
                    <p>Our mission is to make life easier for our customers. We do it by offering easy to use, fast and reliable technology and support services.</p>
                </div>

                <!-- Navigation Links -->
                <div class="footer-nav">
                    <div class="nav-column">
                        <a href="{$WEB_ROOT}/about">About Us</a>
                        <a href="{$WEB_ROOT}/services">Services</a>
                        <a href="{$WEB_ROOT}/ai-protection">AI Protection</a>
                        <a href="{$WEB_ROOT}/managed-services">Managed Services</a>
                        <a href="{$WEB_ROOT}/cyber-security">Cyber Security</a>
                    </div>
                    <div class="nav-column">
                        <a href="{$WEB_ROOT}/whois">WHOIS</a>
                        <a href="{$WEB_ROOT}/domain-tracker">Domain Tracker</a>
                        <a href="{$WEB_ROOT}/terms">Terms of Service</a>
                        <a href="{$WEB_ROOT}/privacy">Privacy Policy</a>
                        <a href="{$WEB_ROOT}/faq">FAQ</a>
                    </div>
                </div>

                <!-- Contact Section -->
                <div class="footer-contact">
                    <h3>Contact Us</h3>
                    <a href="mailto:helpdesk@little.cloud" class="contact-email">helpdesk@little.cloud</a>
                </div>
            </div>
        </div>
    </div>
</section>

</body>
</html> 